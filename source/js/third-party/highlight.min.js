(function (global, factory) {
	...
	*/
	...
	function linenumber(hljs) {
	  ...
	  function initLineNumbersOnLoad(options) {
	    try {
	      ...
	      const blocks = targetElement.querySelectorAll('pre > code');
	  ...
	  function addLineNumbersBlockFor(inputHtml, firstLineIndex) {
	    const lines = getLines(inputHtml); // if last line contains only carriage return remove it
	    ...
	    if (lines.length > firstLineIndex) {
	      ...
	      for (let i = 0, l = lines.length; i < l; i++) {
	        html += format('<tr>' + '<td class="{0}">' + '<div class="{1} {2}" {3}="{5}"></div>' + '</td>' + '<td class="{4}">' + '<div class="{1}">{6}</div>' + '</td>' + '</tr>', [NUMBERS_BLOCK_NAME, LINE_NAME, NUMBER_LINE_NAME, DATA_ATTR_NAME, CODE_BLOCK_NAME, i + 1, lines[i].length > 0 ? lines[i] : ' ']);
	      }
	...
	highlight.registerLanguage('http', http); // 注册代码行数
	const hljsNew = linenumber(highlight);
